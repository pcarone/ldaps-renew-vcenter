#!/bin/bash
# vCenter LDAPS identity source refresh helper
# Uso: copiare nella home del vCenter, adattare le variabili qui sotto oppure lasciarle "vergini"
# e lanciare lo script come root. Se alcuni valori sono vuoti o ancora placeholder, lo script li chiede
# in modo interattivo prima di iniziare.
#
# NOTE IMPORTANTI:
# - Lo script salva lo stato corrente e i certificati live dei DC in una directory timestampata sotto la HOME.
# - Lo script NON salva la password del bind user nel file né nei log; la chiede in modo interattivo.
# - Il comando sso-config.sh richiede comunque le credenziali SSO (es. administrator@vsphere.local)
#   in modo interattivo durante delete/add e, se supportato, durante il set del default identity source.
# - La password del bind user non viene scritta nello script, ma viene comunque passata a sso-config.sh
#   come argomento al momento dell'esecuzione.

set -uo pipefail

############################################
# VARIABILI DA ADATTARE AL PROPRIO AMBIENTE #
############################################
TENANT="vsphere.local"
IDENTITY_SOURCE_NAME="__FILL_ME__"          # Nome esatto dell'identity source esistente da cancellare
DOMAIN_NAME="__FILL_ME__"                   # Es. dominio.io
DOMAIN_ALIAS="__FILL_ME__"                  # Es. DOMINIO
BASE_USER_DN="__FILL_ME__"                  # Es. DC=dominio,DC=io
BASE_GROUP_DN="__FILL_ME__"                 # Es. DC=dominio,DC=io
BIND_USERNAME="__FILL_ME__"                 # Es. CN=svc-vcenter,OU=Service,DC=dominio,DC=io
PRIMARY_LDAPS_URL="__FILL_ME__"             # Es. ldaps://dc1.dominio.io:636
SECONDARY_LDAPS_URL=""                      # Facoltativo. Es. ldaps://dc2.dominio.io:636
DEFAULT_IDENTITY_SOURCE=""                  # Facoltativo. Se vuoto usa DOMAIN_NAME
...
PRIMARY_CHAIN_FILE=""
SECONDARY_RAW_FILE=""
SECONDARY_CHAIN_FILE=""
PRIMARY_CERT_DIR=""
SECONDARY_CERT_DIR=""
ADD_CMD=()
DEFAULT_SET_STATUS="non eseguito"
...
is_placeholder() {
  local value="${1:-}"
  case "$value" in
    ""|"__FILL_ME__"|"CHANGEME"|"changeme"|"dummy"|"DUMMY"|"TODO"|"todo")
      return 0
      ;;
    *)
      return 1
  ...
  while true; do
    if [ "$optional" = "yes" ]; then
      if is_placeholder "$current_value"; then
        read -r -p "$label [invio per lasciare vuoto]: " answer
      else
...
prompt_for_missing_vars() {
  printf '\nCompilazione variabili ambiente\n'
  printf 'Lasciare vuoto un campo facoltativo per non usarlo.\n\n'

  prompt_value TENANT "Tenant SSO" "$TENANT"
  prompt_value IDENTITY_SOURCE_NAME "Nome identity source esistente da cancellare" "$IDENTITY_SOURCE_NAME"
  prompt_value DOMAIN_NAME "Domain name" "$DOMAIN_NAME"
  prompt_value DOMAIN_ALIAS "Alias dominio" "$DOMAIN_ALIAS"
  prompt_value BASE_USER_DN "Base User DN" "$BASE_USER_DN"
  prompt_value BASE_GROUP_DN "Base Group DN" "$BASE_GROUP_DN"
  prompt_value BIND_USERNAME "Bind username" "$BIND_USERNAME"
  prompt_value PRIMARY_LDAPS_URL "Primary LDAPS URL" "$PRIMARY_LDAPS_URL"
  prompt_value SECONDARY_LDAPS_URL "Secondary LDAPS URL" "$SECONDARY_LDAPS_URL" yes
  prompt_value DEFAULT_IDENTITY_SOURCE "Identity source da impostare come default" "$DEFAULT_IDENTITY_SOURCE" yes
  prompt_value SSO_CONFIG_BIN "Percorso sso-config.sh" "$SSO_CONFIG_BIN"

  if is_placeholder "$DEFAULT_IDENTITY_SOURCE" || [ -z "$DEFAULT_IDENTITY_SOURCE" ]; then
    DEFAULT_IDENTITY_SOURCE="$DOMAIN_NAME"
  fi
}
...
validate_vars() {
  ...
  [ -n "$DEFAULT_IDENTITY_SOURCE" ] || fail "DEFAULT_IDENTITY_SOURCE non valorizzato"
...
calc_days_remaining() {
  ...
  if ! end_ts="$(date -u -d "$end_date" +%s 2>/dev/null)"; then
    printf 'N/D'
    return 0
  fi
...
summarize_chain() {
  local url="$1"
  local cert_dir="$2"
  local cert_file idx subject issuer not_before not_after san fingerprint days_left
  ...
  for cert_file in "$cert_dir"/cert-*.crt; do
    ...
    days_left="$(calc_days_remaining "$not_after")"
    printf '  [%s]\n' "$idx"
    printf '    Subject     : %s\n' "$subject"
    printf '    Issuer      : %s\n' "$issuer"
    printf '    Not Before  : %s\n' "$not_before"
...
Dominio            : ${DOMAIN_NAME}
Alias              : ${DOMAIN_ALIAS}
Default finale     : ${DEFAULT_IDENTITY_SOURCE}
============================================================
EOFHDR
}
...
set_default_identity_source() {
  log "Provo a impostare '${DEFAULT_IDENTITY_SOURCE}' come default identity source"
  if "$SSO_CONFIG_BIN" -t "$TENANT" -set_default_identity_sources -i "$DEFAULT_IDENTITY_SOURCE"; then
    DEFAULT_SET_STATUS="OK"
    log "Default identity source impostato correttamente su '${DEFAULT_IDENTITY_SOURCE}'"
    return 0
  fi

  DEFAULT_SET_STATUS="FALLITO"
  warn "Impossibile impostare '${DEFAULT_IDENTITY_SOURCE}' come default tramite sso-config.sh."
  warn "L'identity source LDAPS è stato comunque creato/aggiornato. Se necessario, imposta il default via UI: Administration > Single Sign On > Configuration > Identity Sources > Set as Default."
  return 1
}
...
main() {
  ...
  confirm_or_abort "ATTENZIONE: verrà cancellato l'identity source '${IDENTITY_SOURCE_NAME}', ricreato in LDAPS e poi impostato come default '${DEFAULT_IDENTITY_SOURCE}'. Continuare?"

  run_delete
  run_add
  unset BIND_PASSWORD

  set_default_identity_source || true
  ...
  printf 'Default identity    : %s (%s)\n' "$DEFAULT_IDENTITY_SOURCE" "$DEFAULT_SET_STATUS"
  printf '\n'
